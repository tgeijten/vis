...
namespace vis
{
	...
	VIS_API float perceived_brightness( const color& c );

	VIS_API color make_from_hsv( float H, float S, float V );
	VIS_API color make_unique_color( index_t i, float brightness = 0.6 );
	VIS_API color make_from_hex( unsigned int x );
